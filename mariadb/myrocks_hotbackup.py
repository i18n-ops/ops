...
class BackupRunner:
  ...
  def start_backup_round(self, backup_round, prev_backup):
    ...
    try:
      ...
      dbh = MySQLUtil.connect(opts.mariadb_user,
                              opts.mariadb_password,
                              opts.mariadb_port,
                              opts.mariadb_socket)
...
move_back_usage= "Move-Back: myrocks_hotbackup --move_back --datadir=<dest mariadb datadir> --rocksdb_datadir=<dest rocksdb datadir> --rocksdb_waldir=<dest rocksdb wal dir> --backup_dir=<where backup files are stored> . You need to execute move-back command on a server where backup files are sent."
...
def parse_options():
  ...
  parser.add_option('-c', '--checkpoint_dir', type='string', dest='checkpoint_directory',
                    default='/data/mariadb/backup/snapshot',
  ...
  parser.add_option('-u', '--user', type='string', dest='mariadb_user',
                    default='root',
                    help='MySQL user name')
  parser.add_option('-p', '--password', type='string', dest='mariadb_password',
                    default='',
                    help='MySQL password name')
  parser.add_option('-P', '--port', type='int', dest='mariadb_port',
                    default=3306,
                    help='MySQL port number')
  parser.add_option('-S', '--socket', type='string', dest='mariadb_socket',
                    default=None,
